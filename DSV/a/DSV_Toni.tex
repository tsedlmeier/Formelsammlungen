...
\textbf{Toleranzschema in der log. Notation des \grqq{}filterDesigner\grqq{}: }
  \begin{center}
      \includegraphics[width=.5\textwidth]{./img/toleranzschema1.png}
  \end{center}
  \textbf{Toleranzschema in der lin. Notation: }
  \begin{center}
    \includegraphics[width=.5\textwidth]{./img/toleranzschema.png}
  \end{center}

\begin{itemize}
    \item $A_{pass}=20~log\left(\textstyle{\frac{1+\delta_{D}}{1-\delta_{D}}}\right)=\abs{20\cdot log_{10}(1-\delta_{D})}$
    \item $\delta_{D}= 1- 10^{\frac{-A_{pass}}{20}}$
    \item $A_{Stop}=\abs{20\,log(\delta_{S})}$
    \item $F_{pass}=\frac{\Omega_{D}}{2\pi}f_{A}$
    \item $F_{stop}=\frac{\Omega_{S}}{2\pi}f_{A}$
\end{itemize}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Kanonische-Formen %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Kanonische Struktur: }
\begin{itemize}
...
Für linearphasige reelwertige FIR-Filter kann die Symmetrie der $b_k$ genutzt werden.
  ...
  \end{center}
\subsubsection{SOS-Faustregel (IIR)}
...
\begin{itemize}
    \item \textbf{Linearphasigkeit: }
    	\begin{itemize}
    	\item \textbf{Bedeutung:} linearer Zusammenhang zw. Frequenz und Phasenverschiebung\\
    	=> Gruppenlaufzeit / zeitliche Verzögerung durch Filter IMMER gleich
    	\item \textbf{Voraussetzung:} NST paarweise am Einheitskreis \grqq{}gespiegelt\grqq{} ($z_{01} -> \frac{1}{{z_{01}}^{*}}$)\\
    	ODER NST alle auf Einheitskreis \\(ACHTUNG: Phasensprünge)
    	\end{itemize}
    \item \textbf{Umrechnung Frequenz in Winkel:} $\varphi = \frac{2\cdot\pi\cdot f_{n}}{f_{Abtast}}$
    \item Auf Einheitskreis wird gegen den \textbf{Urzeigersinn}\\
    von 0 Hz (0°) bis $f_{Abtast}$ (360° bzw. $2\pi$) gelaufen
...
\begin{itemize}
    \item \textbf{Besonderheiten: }
         ...
         \item $H(z)=\frac{Y(z)}{U(z)}= \sum_{\mu=0}^{m} b_\mu z^{-\mu}$
         \item m-facher Pol im Ursprung 
         \item Einsatz in adaptiven Systemen
         \item Impulsantwort: gerade \textbf{($h(k)=h(p-k)$)}\\
         oder ungerade \textbf{($h(k)=-h(p-k)$)} Symmetrie
         \item Gruppenlaufzeit:  $\tau_{Gruppe} = -\frac{d}{d\Omega}arg(H(\Omega)) = \frac{p}{2}$ ($p$: Ordnung) 
         \item Impulsantowrt entspricht den b-Koeffizienten in $H(z)$
         \end{itemize}
    \item \textbf{Vorteile: }
   	 \begin{itemize}
    	 \item linearphasig / konstante Gruppenlaufzeit\\
    	 \textbf{WENN} nur gespiegelte NST-Paare: $z_{01}$ und $\frac{1}{z^*_{01}}$
    	 \textbf{ODER} NST auf Einheitskreis
...
\textbf{Grundtypen von FIR-Filtern}\\ (mit entsprechenden Eigenschaften):\\
\textbf{Hinweis: } Symmetrie immer um $\frac{p}{2}$ \textbf{(!!!)}
\begin{center}
      \includegraphics[width=.50\textwidth]{./img/Grundtypen_FIR.png}
      \includegraphics[width=0.25\textwidth]{./img/Grundtypen_FIR_NST.png}
\end{center}
...
\begin{itemize}
    ...
    \item \textbf{Vorteile: }
   	 \begin{itemize}
    	 \item geringere Filterordnung bei vergleichbarem FIR-Filter
    	 \item Entwurfswerkzeuge aus analoger Welt können genutzt werden
    	 \end{itemize}
    \item \textbf{Nachteile: }
    	\begin{itemize}
...
\begin{itemize}
	\item \textbf{Butterworth:}
		\begin{itemize}
		\item monoton fallender Verlauf
		\item maximal flacher Verlauf im Sperrbereich
		\end{itemize}
	\item \textbf{Tschebyscheff Typ I:}
		\begin{itemize}
		\item oszilliert im Durchlassb. mit p Extrema \\
		(p = Filterordnung)\\
    \textbf{ACHTUNG: } jeweils Minimas \& Maximas beachten
		\item monoton fallend im Übergangs‐ \& Sperrbereich
		\end{itemize}
	\item \textbf{Tschebyscheff Typ II:}
		\begin{itemize}
		\item oszilliert im Sperrb. („equiripple“‐Verhalten)
		\item monoton fallend im Durchlassbereich
		\item NST im Sperrbereich
		\end{itemize}
	\item \textbf{Cauer:}
		\begin{itemize}
		\item oszilliert im Durchl. \& Sperrb. („equiripple“‐Verhalten)
		\item monoton fallend im Übergangsbereich
		\item hohe Sperrdämpfung
		\end{itemize}
\end{itemize}
